...
CPipeline2::CPipeline2(VkDevice device, VkRenderPass renderpass, CShaders& shaders) :
    device(device), renderpass(renderpass), 
    pipelineLayout(), graphicsPipeline() {

    descriptorSetLayout = shaders.DescriptorSetLayout();
    shaderStages        = shaders.ShaderStages();
    vertexInputs        = shaders.VertexInputs();
}

CPipeline2::~CPipeline2(){
    if (device) vkDeviceWaitIdle(device);
    //if (descriptorSetLayout) vkDestroyDescriptorSetLayout(device, descriptorSetLayout, nullptr);
    if (pipelineLayout)      vkDestroyPipelineLayout     (device, pipelineLayout,      nullptr);
    if (graphicsPipeline)    vkDestroyPipeline           (device, graphicsPipeline,    nullptr);
...
VkPipeline CPipeline2::CreateGraphicsPipeline(VkExtent2D extent) {
    VkPipelineInputAssemblyStateCreateInfo inputAssembly = {VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO};
    inputAssembly.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
    inputAssembly.primitiveRestartEnable = VK_FALSE;
    ...
    pipelineInfo.stageCount = 2;
    pipelineInfo.pStages = shaderStages;
    pipelineInfo.pVertexInputState = vertexInputs; //&vertexInputInfo;
    pipelineInfo.pInputAssemblyState = &inputAssembly;
    //pipelineInfo.pTessellationState = 0;
    pipelineInfo.pViewportState = &viewportState;
    pipelineInfo.pRasterizationState = &rasterizer;
